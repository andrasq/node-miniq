...
// FIXME: this.emitStats('HttpRunner.runJobs start', { id: job.id, type: job.type });
        ...
        microreq(self._makeUri(handler.body), job.data, landJob);
...
// FIXME: this.emitStats('HttpRunner.runJobs stop', { id: job.id, type: job.type, ms: durationMs });
            if (err) {
                job.exitcode = res && res.statusCode || 500;
                job.code = err.code;
            } else {
