...
function HandlerStore( store ) {
    this.handlerExpireAfter = new Date('3000-01-01');   // handler expiration timestamp
    this.handlerDeleteBefore = new Date('3099-01-01');  // handler delete-if-expires-before timestamp
    this.store = store;
}
...
HandlerStore.prototype.deleteHandler = function deleteHandler( jobtype, callback ) {
    this.store.expireJobs(jobtype, Store.LOCK_HANDLER, this.handlerDeleteBefore, 999999, callback);
}
