...
// TODO: change to MiniLogger that takes (string [,object]) instead of varargs
// TODO: or maybe change prototype to (format, ...args) | (format, infoObject)
function makeLogger( logName, stream ) {
    ...
    var loglineTemplate = ('{"time":"%s","type":"%s","id":' + JSON.stringify(logName) + ',"message":%s}\n').slice(0);
    function vlogit(argv, level) {
        ...
        var logline = util.format(loglineTemplate, getNewerTimestring(0), level, JSON.stringify(msg));
...
function getId( sysid ) {
    ...
    if (ts > _idTimestamp) { _idPrefix = ''; if (_idSequence > _idSequenceLimit * .5) { _idSequence = 0 } }
    ...
    var id = _idPrefix + _charset64[_idSequence++ & 0x3f];

    if ((_idSequence & 0x3f) === 0) {
        _idPrefix = '';  // when last digit overflows signal need for reformat
        if (_idSequence >= _idSequenceLimit) _idSequence = 0; // when sequence overflows signal need for new timestamp
    }
    return id;
}
...
function Mutex() {
    ...
    this.release = function release() {
        self.busy = false;
        if (self.queue.length) utils.setImmediate(self.queue.shift(), release, self.busy = true);
    }
}
