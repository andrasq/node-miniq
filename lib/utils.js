...
function makeNoopLogger( id, stream ) {
    var noop = function(){};
...
var _ncalls = 0;
var _timeout;
function _clearLastTime() { _lastTimestamp = -1; _timeout = null }
...
function getId( sysid ) {
    ...
    else if (!_idSequence) ts = getNewerTimestamp(_idTimestamp); // if at end of seq, we need a newer timestamp to start a new sequence
    _idTimestamp = ts;
...
KeyedSet.prototype.set = function set(k, v) { this.items[k] = v };
KeyedSet.prototype.has = function has(k) { return this.items[k] !== undefined };
KeyedSet.prototype.get = function get(k) { return this.items[k] };
KeyedSet.prototype.delete = function get(k) { this._dirty += 1; delete this.items[k] };
// NOTE: on some versions, delete can be slow
// KeyedSet.prototype.delete = function get(k) { this._dirty += 1; this.items[k] = undefined };
KeyedSet.prototype.keys = function keys() { this.gc(); return definedKeysOf(this.items) };
KeyedSet.prototype.values = function keys() { this.gc(); return definedValuesOf(this.items) };
KeyedSet.prototype.gc = function gc() { (this._dirty > 200) && (this.items = gcTo({}, this.items)) && (this._dirty = 0) };
KeyedSet.prototype = toStruct(KeyedSet.prototype);
**/
