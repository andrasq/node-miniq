...
function makeLogger( name, stream ) {
    var levelNos = { LOG: -1, ERROR: 3, WARN: 4, INFO: 6, DEBUG: 7, TRACE: 8,
                     log: -1, error: 3, warn: 4, info: 6, debug: 7, trace: 8, };
    ...
    function vlogit(argv, level) {
        ...
        var logline = util.format(loglineTemplate, new Date(getNewerTimestamp(0)).toISOString(), level, name,
    ...
    return { trace: utils.varargs(vlogit, 'TRACE'), debug: utils.varargs(vlogit, 'DEBUG'),
             ...
             error: utils.varargs(vlogit, 'ERROR'),
               log: utils.varargs(function(argv) { vlogit([utils.invoke(util.format, argv)], 'LOG') }) };
}
...
function Cron( ) {
    ...
    this.schedule = function schedule(interval, fn, errorCallback) {
        // if (typeof at !== 'number') at = this._parseTime(at);
