...
function Cron( ) {
    ...
    this.schedule = function schedule(interval, fn, errorCallback) {
        ...
        this.jobs.push({ interval: interval, last: 0, next: this._findNextTime(Date.now(), interval), fn: fn, ecb: errorCallback });
        return this;
    }
    this.run = function run(now, callback) {
        ...
        utils.repeatUntil(function(done) {
            ...
            jobs[i].next = self._findNextTime(now, jobs[i].interval);
    ...
    this._findNextTime = function _findNextTime( now, interval ) {
