...
// interval jobs, launched at fixed intervals relative to now, non-overlapping
function Cron( ) {
    this.jobs = [];
}
Cron.prototype.schedule = function schedule(_interval, fn, errorCallback) {
    var now = Date.now(), interval = utils.parseTimeInterval(_interval);
    if (isNaN(interval)) throw new Error('invalid interval, expected [0-9]+[hms]');
    this.jobs.push({
        interval: interval, start: now, next: this._findNextRunTime(now, now, interval), fn: fn, errCb: errorCallback });
}
Cron.prototype.cancel = function cancel(fn) {
    this.jobs = this.jobs.filter(function(job) { return job.fn !== fn });
}
Cron.prototype.run = function run(now, callback) {
    var jobIdx = 0, jobs = this.jobs, self = this, doneCb = null;
    function fnCb(err) {
        jobs[jobIdx].next = self._findNextRunTime(now, jobs[jobIdx].start, jobs[jobIdx].interval);
        if (err && jobs[jobIdx].errCb) jobs[jobIdx].errCb(err); doneCb() }
    utils.repeatUntil(function(done) {
        if (jobIdx >= jobs.length || jobs[jobIdx].next > now) { return done(null, ++jobIdx >= jobs.length); }
        jobs[jobIdx].next = Infinity, doneCb = done, jobs[jobIdx].fn(fnCb);
    }, function(err, isDone) { callback(err) });
}
Cron.prototype._findNextRunTime = function _findNextRunTime( nowMs, startMs, interval ) {
    var msRunning = nowMs - startMs;
    return nowMs + (interval - (msRunning % interval));
}
var _timeUnits = { w: 7*24*3600*1000, d: 24*3600*1000, h: 3600*1000, m: 60*1000, s: 1000, '': 1 };
function parseTimeInterval( interval ) {
    if (+interval == interval) return +interval;
    var parts = String(interval).match(/^\s*(\d+\.?|\d+\.\d+|\.\d+)\s*([wdhms]?)\s*$/) || [];
    return Number(parts[1]) * _timeUnits[parts[2]]; // number or NaN
}
...
// NOTE: the one in qibl is better
...
// high-res timer -- for a carefully calibrated version like php see microtime() in qibl
function microtime( ) {
