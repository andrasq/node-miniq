...
function Cron( ) {
    ...
    this.schedule = function schedule(interval, fn) {
        ...
        this.jobs.push({ interval: interval, last: 0, next: findNextTime(Date.now(), interval), fn: fn });
        return this;
    }
    this.run = function run(now, callback) {
        ...
        repeatUntil(function(done) {
            ...
            jobs[i].fn(function(err) { done(err, ++i >= jobs.length) });
        }, callback);
    }
...
function abstract( name ) {
    ...
    var src = 'function ' + name + '(' + args.join(', ') + ') {\n' +
