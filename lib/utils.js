...
var _ncalls = 0;
var _timeout;
function _clearLastTime() { _lastTimestamp = -1; _timestampString = ''; _timeout = null }
...
function getId( sysid ) {
    ...
    // when sequence overflows, wait for a new timestamp
    if ((_idSequence & 0x3f) === 0) {
        _idPrefix = '';  // signal need for reformat
        if (_idSequence >= _idSequenceLimit) _idSequence = 0; // signal need for new timestamp
    }
    return id;
}
