...
function _tryRequire(pathname) { try { return global.require(pathname) } catch (err) {} }
function _tryLoad(loader, file) { try { return loader(file) } catch (err) {} }
function getConfig( configDir, loaders ) {
    if (!loaders && typeof configDir !== 'string') { loaders = configDir; configDir = null }
    configDir = configDir || __dirname + '/../config';
    loaders = loaders || {};
    var env = process.env.NODE_ENV || 'development';
    var filepath = configDir + '/' + env;
    var pkg = _tryRequire(filepath + '') || _tryRequire(filepath + '.js') || _tryRequire(filepath + '.json');
