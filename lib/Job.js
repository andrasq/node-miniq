 ...
 *    dt        - timestamp or null.  For unclaimed jobs, when they are eligible to run.  Not null;
 ...
 *    create_dt
 *    done_dt
 *    busy      - boolean isLocked info, true if lock <> '', else false
 *
...
Job.fromJournalLine = function fromJournalLine( line ) {
    ...
    return match ? new Job(match[1], match[2], new Date(utils.decode64(match[1])), match[3]) : null;
}
