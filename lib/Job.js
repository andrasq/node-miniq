 ...
 *                with the job completion time being 1000 years less than job.dt
...
Job.fromJournalLine = function fromJournalLine( line ) {
    ...
    // CAUTION: if the id timestamp is old, the job could expire sooner than expected
    return new Job(match[1], match[2], new Date(ms), match[3]);
}

Job.getLineId = function getLineId( line ) {
...
Job.getCreateDt = function getCreateDt( job ) {
    return new Date(utils.decode64(job.id));
}
