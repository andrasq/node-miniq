...
MockStore.prototype.releaseJobs = function releaseJobs( ids, lock, how, callback ) {
    ...
    utils.filterUpdate(this.jobs,
        ...
        { dt: newDt, lock: newLock }, {}, callback);
}
...
MockStore.prototype.expireJobs = function expireJobs( jobtype, lock, cutoffDt, limit, callback ) {
    if (!jobtype && !lock) throw new Error('jobtype or lock required');
    utils.filterUpdate(this.jobs,
        function(job) { return ((job.lock === lock) || !lock) && ((job.type === jobtype) || !jobtype) && job.dt < cutoffDt },
