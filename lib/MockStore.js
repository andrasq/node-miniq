...
MockStore.prototype.getWaitingJobtypes = function getWaitingJobtypes( callback ) {
    ...
    this.jobs.forEach(function(job) {
        if (job.dt <= dt && job.lock === Store.LOCK_NONE) types[job.type] = true;
    })
    callback(null, Object.keys(types));
}

MockStore.prototype.getRunningJobtypes = function getRunningJobtypes( callback ) {
    var types = {};
    this.jobs.forEach(function(job) {
        if (job.lock !== Store.LOCK_NONE && job.lock !== Store.LOCK_DONE && job.lock !== Store.LOCK_HANDLER) {
            types[job.type] = true;
        }
    })
    callback(null, Object.keys(types));
...
MockStore.prototype.getWaitingJobcounts = function getWaitingJobcounts( callback ) {
    ...
    var types = {};
    this.jobs.forEach(function(job) {
        if (job.dt <= dt && job.lock === Store.LOCK_NONE) types[job.type] = types[job.type] + 1 || 1;
    })
    callback(null, types);
}
...
MockStore.prototype.getJobs = function getJobs( jobtype, limit, lock, expireMs, callback ) {
    ...
    utils.filterUpdate(this.jobs,
        function select(job) { return job.type === jobtype && job.lock === Store.LOCK_NONE && job.dt <= nowDt },
...
MockStore.prototype.releaseJobs = function releaseJobs( ids, lock, how, callback ) {
    var newLock = how === 'archive' ? Store.LOCK_DONE : Store.LOCK_NONE;
...
MockStore.prototype.expireLocks = function expireLocks( callback ) {
    var nowDt = new Date(Date.now());
    utils.filterUpdate(this.jobs,
        function(job) { return job.lock !== Store.LOCK_NONE && job.dt < nowDt }, { dt: nowDt, lock: Store.LOCK_NONE }, null, callback);
}
...
MockStore.prototype.expireJobs = function expireJobs( jobtype, lock, cutoffMs, limit, callback ) {
    ...
    if (typeof jobtype !== 'string' && typeof lock !== 'string') throw new Error('jobtype or lock required');
    utils.filterUpdate(this.jobs,
...
MockStore.prototype.getNewestByType = function getNewestByType( jobtype, lock, callback ) {
// TODO: ?? pass in limit, return the N newest sorted ORDERED BY `dt` DESC
    var limit = 1;
    var newest;
    utils.filterUpdate(this.jobs,
        function(job) { return (job.type === jobtype && job.lock === lock) },
        null, {}, function(err, jobs) { callback(err, jobs && jobs.sort(desc).slice(0, 1)) });
    function desc(a, b) { return a.dt >= b.dt ? -1 : 1 }
}
