 ...
 * The optional callback is called once addedLines have been recorded in the journal (persisted).
 */
Journal.prototype.write = utils.abstract('write', 'addedLines: string | string[]', 'callback?');
 ...
 * The actual number of lines returned might be less than lineCount.
 * Reserved lines are available for reading only with the returned token.
 * The ticketing system is used to track which journal lines have been ingested
 * and which need to be read again.
 */
 ...
 */
Journal.prototype.rsync = utils.abstract('rsync', 'token', 'callback');
