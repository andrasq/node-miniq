...
JournalArray.prototype.wsync = function wsync( callback) {
    ...
    var self = this;
// TODO: move the controlling logic into Journal.js, have only the sync implementation in **Journal.js
    if (!this.wsyncTimer) this.wsyncTimer = setTimeout(function() {
        ...
        // err = wait for in progress writes to finish
...
JournalArray.prototype.readReserve = function readReserve( nlines, timeoutMs ) {
    ...
    lines.timer.unref && lines.timer.unref();
    return token;
}
 ...
 */
JournalArray.prototype.rsync = function rsync( token, callback ) {
    var lines = this.reservations[token];
    if (lines) {
