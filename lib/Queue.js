...
function Queue( sysid, journal, scheduler, jobStore, runner, log ) {
    ...
    this._doneJobExpireMs = 14 * DAYS;
    this._unrunJobExpireMs = 2 * DAYS;
...
next(null, 'function jobHandler(){}');
        ...
        function(next, _handler) {
            handler = _handler;
            self.store.getJobs(jobtype, self.runner.getBatchSize(jobtype), self.sysid, self._lockExpireMs, next);
        },
        function(next, jobs) {
            this.stats.emit('runNewJobs', jobtype, jobs.length);
...
Queue.prototype._renewLocks = function _renewLocks( callback ) {
    ...
    utils.iterateSteps([
        ...
        function(next, runningJobIds) {
            ...
            this.stats.emit('renewLocks', runningJobIds);
            self.store.renewLocks(runningJobIds, self.sysid, self._lockExpireMs, next);
        }
    ], callback)
}

Queue.prototype._expireLocks = function _expireLocks( callback ) {
    this.stats.emit('expireLocks');
    this.store.expireLocks(callback);
}

Queue.prototype._expireJobs = function _expireJobs( callback ) {
    var self = this;
    this.stats.emit('expireJobs');
    utils.iterateSteps([
        function(next) {
            self.store.expireJobs(null, Store.LOCK_DONE, this._doneJobExpireMs, 100000, next);
        },
        function(next) {
            self.store.expireJobs(null, Store.LOCK_NONE, this._unrunJobExpireMs, 100000, next);
        },
    ], callback);
}
