...
function Queue( sysid, journal, scheduler, jobStore, runner, log ) {
    ...
    this.cron.schedule('29s', function(cb) { self._renewLocks(cb) });
    this.cron.schedule('11s', function(cb) { self._expireLocks(cb) });
    this.cron.schedule('67s', function(cb) { self._expireJobs(cb) });
...
Queue.prototype.runNewJobs = function runNewJobs( callback ) {
    ...
    utils.iterateSteps([
        ...
        function(next, jobtype) { self.store.getJobs(jobtype, self.runner.getBatchSize(jobtype), self.sysid, self._lockExpireMs, next) },
        function(next, jobs) {
