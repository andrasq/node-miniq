...
function Queue( sysid, journal, scheduler, jobStore, systemStore, runner, log ) {
    ...
    this.store = jobStore;
// TODO: pass in an array of job stores?
...
Queue.prototype.handleDoneJobs = function handleDoneJobs( callback ) {
    ...
    self.runner.getDoneJobs(function(err, jobtype, doneJobs) {
        ...
        self.scheduler.jobsStopped(jobtype, jobIds.length);
...
Queue.prototype.addJobs = function addJobs( jobtype, lines, callback ) {
// FIXME: try not to double-convert request buffer -> chars -> buffer 
