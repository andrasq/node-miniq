...
function makeRoutes( router, app ) {
    ...
    router.setRoute('/add', 'POST', function(req, res, next) {
        ...
        app.queue.addJobs(lines, function(err) {
console.log("AR: added job, journal now", app.queue.journal);
            res.end(restMw.sendResponse(req, res, next, err, 200, { ok: true, count: lines.length }));
        })
    })
