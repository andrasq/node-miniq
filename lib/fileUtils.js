...
var removeTrailingNewline = eval('true && nodeVersion < 0.8001 ? function(line) { return line.slice(0, -1) } : function(line) { return line }');
...
// FIXME: need to track and know the current byte read offset
    ...
    var reader = {
        ...
        error: null,
        rl: rl,
        isEof: function() { return !reader.lines.length && (reader.eof || reader.error) },
        gets: function gets() {
    ...
    reader.rl.on('line', function(line) { if (reader.lines.push(removeTrailingNewline(line)) >= lineBufLength) { reader.rl.pause(); reader.paused = true } });
...
function waitWhile( test, timeoutMs, callback ) {
    ...
    var timeout = false;
    var timer = timeoutMs > 0 && setTimeout(function() { timeout = true }, timeoutMs);
    setTimeout(retry, 0);

    function retry() {
        timeout ? callback(new Error('wait timeout: ' + test))
